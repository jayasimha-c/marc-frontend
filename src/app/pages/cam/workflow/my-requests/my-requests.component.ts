...
@Component({
  standalone: false,
  selector: 'app-wf-my-requests',
  templateUrl: './my-requests.component.html',
  encapsulation: ViewEncapsulation.None,
})
export class WfMyRequestsComponent implements OnInit {
  data: any[] = [];
  totalRecords = 0;
  loading = true;
  ...
  columns: TableColumn[] = [
    ...
    { header: 'Actions', field: 'actions', type: 'actions', width: '130px',
  ...
  loadData(event: TableQueryParams): void {
    ...
    this.loading = true;
    this.workflowService.getMyRequests(event, this.search).subscribe({
      next: (resp) => {
  ...
  exportExcel(): void {
    this.workflowService.exportMyRequests(this.search).subscribe({
      next: (resp) => this.fileSaverService.saveAnyFile(resp, 'My_Requests.xlsx'),
      error: (err) => this.notificationService.handleHttpError(err),
    });
  }
}
